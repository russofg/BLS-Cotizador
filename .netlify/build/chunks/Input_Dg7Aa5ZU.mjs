...
const $$Input = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="w-full"> ${label && renderTemplate`<label${addAttribute(id, "for")} class="block text-sm font-medium text-gray-700 dark:text-gray-300"> ${label} ${required && renderTemplate`<span class="text-red-500 dark:text-red-400 ml-1">*</span>`} </label>`} <input${addAttribute(type, "type")}${addAttribute(name, "name")}${addAttribute(id, "id")}${addAttribute(placeholder, "placeholder")}${addAttribute(value, "value")}${addAttribute(required, "required")}${addAttribute(disabled, "disabled")}${addAttribute(readonly, "readonly")}${addAttribute(min, "min")}${addAttribute(max, "max")}${addAttribute(step, "step")}${addAttribute(autocomplete, "autocomplete")}${addAttribute(inputClasses, "class")}> ${helperText && !error && renderTemplate`<p class="mt-1 text-sm text-gray-500 dark:text-gray-400">${helperText}</p>`} ${error && renderTemplate`<p class="mt-1 text-sm text-red-600 dark:text-red-400">${error}</p>`} </div>`;
}, "/Users/fernandogabrielrusso/Desktop/Cotizador-Online/cotizador/src/components/ui/Input.astro", void 0);
